<template>
	<view class="banner">
		...
		<view class="bar,barb">
			<uni-section v-if="data.showsubmits" title="提交情况">
				<template v-slot:right>
					<uni-icons @click="overview" type="refreshempty" size="18"></uni-icons>
				</template>
				<view v-if="submitCounts.numOfAnswers>0">
					<text>总提交份数：{{submitCounts.numOfAnswers}}</text>
					...
					<view  v-if="data.table">
						<uni-table border stripe emptyText="暂无更多数据" >
							...
							<uni-tr v-for="(item,index) in submitCounts.answerCountList" :key="index">
								...
								<uni-td>
									<uni-tr v-for="(itemofc,indexofc) in item.choiceSumList" :key="indexofc">{{Number(itemofc)/submitCounts.numOfAnswers*100}}%</uni-tr>
								</uni-td>
					...
					<view v-else v-for="(item,index) in data.chartDatas" :key="index">
						...
						<view class="charts-box" style="top: 2px;" :key="data.chartDatas[index].key">
							{{index+1}}.{{data.chartDatas[index].q.type}}:{{data.chartDatas[index].q.name}}：
							<qiun-data-charts v-if="data.chartDatas[index].series[0].data.length>0" type="pie" :opts="data.opts" :chartData="data.chartDatas[index]"></qiun-data-charts>
						</view>
						</transition>
					</view>
				</view>
...
const overview = ()=>{
		http('/questionnaire/useranswer/anssum?questionnaireId='+newNaire.id,'GET',{},).then((res)=>{
			submitCounts.value = {...res.data}
			data.chartDatas=[]
			if(submitCounts.value.numOfAnswers){
				for (var i = 0; i < submitCounts.value.answerCountList.length; i++) {
					let datai=[]
					data.chartDatas.push({
					  q: data.questionList.filter((q)=>{return q.id === submitCounts.value.answerCountList[i].questionId})[0],
					  key: Date.now(),
					  series: [
						{
						  data: datai
					...
					if(data.chartDatas[i].q.type.includes('选')){
						for (var j = 0; j < submitCounts.value.answerCountList[i].choiceSumList.length; j++) {
							datai.push({"name":data.chartDatas[i].q.content[j].text,"value":Number(submitCounts.value.answerCountList[i].choiceSumList[j])})
						}
					}
				}
			}
