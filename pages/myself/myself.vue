	...
	<view v-else style="padding-top: 10rpx;background: #597fbe;">
		...
		<view class="User">
			<uni-file-picker limit="1" @select="selectUpload" file-mediatype="image" title="" disable-preview
				ref="uniFilePicker" return-type="object" :modelValue="filelist" :imageStyles="imageStyles"
				:del-icon='false' required>
			</uni-file-picker>
		...
		<uni-list class="info-bar">
			...
			<uni-list-item title="班级"
				:rightText="store.user.classInfo.majorName+store.user.classInfo.gradeName+store.user.classInfo.className" />
		...
		<view class="btnview">
			<button type="primary" class="btn" style="background: #d6d6d69e;color: #616161;"
				@click="()=>{store.delogin();}">
				<text>退出登录</text>
			</button>
		</view>
