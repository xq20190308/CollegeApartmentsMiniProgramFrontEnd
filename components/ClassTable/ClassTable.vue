		...
		<div class="table-wrapper">
			<div class="table-container">
				<div class="class-table">
					<div class="thead">
						<div class="tr">
							<div class="th" v-for="(item, index) in classTableData.weeks" :key="index">{{ item ? '周' + item : '' }}
							</div>
						</div>
					</div>
					<div class="tbody">
						<div class="tr" v-for="(item, index) in classTableData.courses" :key="index">
							<div class="td" style="font-size:12px;background:#d4f7fd;word-wrap: break-word; 
							                    word-break: break-all; ">
								{{ changeCharacter(index) }}
							</div>
							<div class="td" v-for="(innerItem, idx) in item" :key="idx" @click="toScanDetail(innerItem, idx)">
								<div v-if="innerItem.lessonsName">
