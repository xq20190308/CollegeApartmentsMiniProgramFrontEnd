      ...
      await ((_a = req.value) == null ? void 0 : _a.validate().then(async (res1) => {
        ...
        await pages_login_api_login.login(data.reqdata).then(async (res) => {
          if (res.statusCode == 200) {
            ...
            try {
              await utils_cache.setUserInfo(res);
            } catch (e) {
              console.log("set不对", e);
            }
            const res12 = await utils_http.http("/user/getavatar", "GET", {});
            utils_cache.setLocalData("avatarUrl", res12.data);
            try {
              utils_cache.getLocalAll();
            } catch (e) {
              console.log("get不对", e);
            }
            show.value = false;
